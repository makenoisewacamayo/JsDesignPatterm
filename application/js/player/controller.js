define([
    ...
    function(PlayerProgressBar, D$, Timer, PubSub, Injector) {
        ...
        var playerButtonPlay = document.getElementById("player-button-play");
        var playerButtonPlayIcon = document.getElementById("player-button-play-icon");
        var playerState = {
            track: null,
            isPlaying: false,
            progress: 0,
        ...
        function playTrack(track) {
            // Do nothing if is the same track and player is already playing

            if (playerState.track && track.id === playerState.track.id && playerState.isPlaying) return false;
            if (!track.isPlayable) return PubSub.trigger('reuest:queue:next');

            // if same track, just play forwaad
        ...
        var renderTrackAdditionalInfo = Injector.resolve([
            ...
            'track/track_data'
        ], function(HeroTrackView, HeroSingleView, TrackData, track) {
            TrackData(track)
                ...
                .catch(function(err) {
                    ...
                    console.log("Was not able to get addicional track information for ID=" + track.id);
                });
        });
        ...
        PubSub.on('request:player:pause', function(trackData) {
            pauseTrack();
        });
    });
